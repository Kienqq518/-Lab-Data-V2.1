...
window.WEB_ANNOTATION_REGISTRY['web-settings'] = {
  sampleCodeConversion: {
    title: '启用样品编号转换',
    requirementLogic:
      '对接第三方 LIMS 时，原样品编号可能含汉字、特殊符号或超长字符串，上位机与设备直连程序无法稳定识别。开启后，后端在任务/样品同步阶段调用编号转换接口：提取原编码中的字母数字片段，拼接为唯一标准 ID（如 SC2026/01001-01、LIMS/20260720/001-02）；允许字符为 A–Z、a–z、0–9 及 /、-、_，且分隔符仅出现在片段之间，不得连续出现（禁止 -_-_-、/_ / 等）。标准 ID 写入映射表，App 与上位机交互一律使用标准 ID；App 端展示「转换后编号（原编号）」便于现场核对。关闭时全链路直接使用 LIMS 原编号，适用于数蚕 LIMS 直连或编号已规范的场景。本开关按检测机构生效，检测员 App 只读同步，不可在移动端修改。',
    displayRule:
  ...
  sampleCodePreview: {
    ...
    interactionLogic:
      '只读演示，无排序、筛选、编辑、导出。开关关闭时整块隐藏。Toggle 开启后若预览区因 DOM 更新需重算位置，批注模式自动刷新锚点。评审时配合左侧 sampleCodeConversion 批注一并讲解。',
  },
};
