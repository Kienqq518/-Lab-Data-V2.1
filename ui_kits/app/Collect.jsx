...
(function () {
  const { AppBar, FieldRow, Button, CollectBadge, UploadStatus, Card } = window.DesignSystem_52fd11;
  const M = window.MOCK;
  ...
  function Collect({ ctx, onBack, onDone }) {
    ...
    if (ctx.item && ctx.item.subs && ctx.item.subs.length && window.CollectStructured) {
      return <window.CollectStructured ctx={ctx} onBack={onBack} onDone={onDone} />;
    }
