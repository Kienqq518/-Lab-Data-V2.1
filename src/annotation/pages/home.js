...
export const homeAnnotations = {
  notifyBell: {
    ...
    interactionLogic: '点击后打开消息通知全屏 overlay：先进入通知列表，点击单条进入详情。通知按委托任务为单位；详情页「去处理」深链到对应任务 L3，展示该任务下分配给当前检测员的样品试验项。',
  },
  companyHeader: {
    title: '公司标题栏',
  ...
  quickEntrySection: {
    ...
    interactionLogic: '待检 → pending 聚焦页 L2（列表含已逾期但未开检的任务，不含 status=done 已检归档，支持扫码进 L3）；检测中 → testing 聚焦页 L2（列表含已逾期但仍在检的任务，不含已检归档）；已检 → 打开「已检任务」overlay（近 30 天已完成任务复核列表，L4 流程未锁定时可编辑补传）。',
  },
  inspectorStats: {
    title: '个人检测统计',
