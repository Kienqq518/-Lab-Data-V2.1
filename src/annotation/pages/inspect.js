...
export const inspectAnnotations = {
  modeSwitch: {
    ...
    displayRule: 'SegmentedSwitch 两段；切换时清空搜索词、重置 L2/L3 钻取栈、清除采集方式筛选。按任务模式隐藏右上角工位栏，仅显示任务总数。',
  ...
  l3SearchBar: {
    title: 'L3 搜索（样品/试验项）',
    requirementLogic: 'L3 支持按样品名称、样品编号、试验项名称联合过滤。规则：① 关键词命中样品名称或编号时，仅过滤左侧样品卡片列表，右侧展示当前选中样品下的试验项（若试验项名也命中则再收窄右侧）；② 关键词仅命中试验项名称时，左侧仅保留含该试验项的样品，右侧仅展示匹配的试验项卡片。不支持扫码。',
  ...
  taskList: {
    title: '委托任务列表（L1 按任务）',
    requirementLogic: '按任务模式下的 L2 入口：展示全部可检委托任务，含逾期、待检、检测中状态。支持排序与关键词过滤。',
  ...
  deviceTaskList: {
    title: '设备相关任务（L2 按设备）',
    requirementLogic: '仅展示下属样品/试验项涉及当前设备的委托任务；排序与 L1 按任务一致。',
  ...
  sampleSidebar: {
    ...
    displayRule: '160px 按钮列表：StatusTag、序号、样品编号、样品名两行截断；选中左蓝条+浅底；扫码样品加蓝色描边高亮。',
