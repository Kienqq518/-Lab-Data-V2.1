...
/**
 * L2 任务列表排序控件（各聚焦页共用）
 * 触发按钮展示当前排序，点击后从底部弹出排序面板（挂载到 .screen 避免被裁剪）
 */
export function TaskListSort({ value, onChange, options = TASK_SORT_OPTIONS, style }) {
  ...
  React.useLayoutEffect(() => {
    ...
    const root = triggerRef.current?.closest('.screen, .overlay-screen');
  ...
  const sheet = open && portalRoot ? (
    <>
      <div
        ...
        style={{ position: 'absolute', inset: 0, zIndex: 80, background: 'rgba(15,23,42,0.28)' }}
      />
      <div
        className="task-sort-sheet"
        style={{
          position: 'absolute', left: 0, right: 0, bottom: 0, zIndex: 90, background: 'var(--white)',
          borderRadius: '18px 18px 0 0', boxShadow: '0 -14px 34px rgba(15,23,42,0.20)',
      ...
      >
        ...
        <div style={{ padding: '8px 16px 12px', borderBottom: '1px solid var(--divider)', flexShrink: 0 }}>
        ...
        <div style={{ padding: '10px 16px 16px', borderTop: '1px solid var(--divider)', flexShrink: 0 }}>
  ...
  return (
    <>
      ...
      {sheet && createPortal(sheet, portalRoot)}
    </>
  );
}
